...
class GetDiscoverMoviesUseCaseTest {
    ...
    fun setUp() {
        ...
        useCase = GetDiscoverMoviesUseCase(repository)
    }

    @Test
    fun `invoke returns discover movies for given genre`() = runTest {
        val result = useCase(genreId = 28, page = 1).first()
    ...
    @Test
    fun `invoke returns discover movies for all genres when genreId is null`() = runTest {
        val result = useCase(genreId = null, page = 1).first()
        assertTrue(result is Resource.Success)
        val movies = (result as Resource.Success).data
        assertEquals(1, movies?.size)
        assertEquals("Test Movie", movies?.first()?.title)
